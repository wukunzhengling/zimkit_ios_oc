...
- (void)layoutSubviews {
    ...
    if (self.coverViews) {
        ...
        self.downloadBtn.frame = CGRectMake(0, self.contentView.height -60 - Bottom_SafeHeight, self.contentView.width, 60);
    }
...
- (void)downloadAction {
    if ([self.downloadBtn.button.titleLabel.text isEqualToString:[NSBundle ZIMKitlocalizedStringForKey:@"message_album_download_image"]]) {
        ...
        if (photoView == nil) {
            ...
            photoView.loadFailed = ^(GKPhotoView * _Nonnull curPhotoView) {
                ...
                if (self.loadStyle == GKPhotoBrowserLoadStyleKit) {
                    ...
                    [weakSelf.downloadBtn.button setTitle:[NSBundle ZIMKitlocalizedStringForKey:@"message_album_redownload_image"] forState:UIControlStateNormal];
                }
            ...
            photoView.loadComplete = ^(BOOL scuess) {
                if (self.loadStyle == GKPhotoBrowserLoadStyleKit) {
                    weakSelf.downloadBtn.hidden = NO;
                    if (scuess) {
                        [weakSelf.downloadBtn.button setTitle:[NSBundle ZIMKitlocalizedStringForKey:@"message_album_download_image"] forState:UIControlStateNormal];
                    } else {
                        [weakSelf.downloadBtn.button setTitle:[NSBundle ZIMKitlocalizedStringForKey:@"message_album_redownload_image"] forState:UIControlStateNormal];
                    }
                }
...
- (ZIMKitImageDownload *)downloadBtn {
    if (!_downloadBtn) {
        _downloadBtn = [[ZIMKitImageDownload alloc] init];
        _downloadBtn.hidden = YES;
        @weakify(self);
        _downloadBtn.downloadActionBlock = ^{
            @strongify(self);
            [self downloadAction];
        };
    }
